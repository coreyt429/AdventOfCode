...
def anonymize(floors):
    ...
    # fast but wrong.  I suspect we are anonymizing too much
...
def try_move_a_star(current_node, items, new):
    ...
    if (is_valid(new['floors'][current_node.floor]) and
        ...
        new_nodes.append(Node(
            ...
            h_score(tuple(tuple(list(floor)) for floor in new['floors']), current_node.g_score, current_node.threshold),
            current_node
        )
        )
    return new_nodes
...
def solve_a_star(floors):
    ...
    start_node = Node(0, tuple(tuple(list(floor)) for floor in floors), 0, h_score(tuple(tuple(list(floor)) for floor in floors), 0, min_solved))
