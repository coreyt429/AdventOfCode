...
def set_valve_distances():
    ...
    # init graph
    graph = [
        [float("infinity") for _ in range(len(valve_names))]
...
def potential_pressure(minutes, closed_valves, time_limit=30):
    ...
    sorted_valves = sorted(
        (
            valves[valve_id]["flow_rate"]
            for valve_id in closed_valves
            if valve_id != "DONE"
        ),
        reverse=True,
    )
    ...
    for flow_rate in sorted_valves:
        ...
        if worker_time < time_limit:
            ...
            available_workers[0] += (
                2  # One minute to open the valve, and one to move to the valve
            )
        else:
...
def max_pressure_relief(working_valves, time_limit=30, workers=1):
    ...
    while heap:
        ...
        if any(minute >= time_limit for minute in current["minutes"]):
            current["locations"] = mark_done(
                current["locations"], current["minutes"], time_limit
            )
        ...
        if pressure > stats["pressure"]:
            ...
            pressure
            + potential_pressure(
                current["minutes"], current["closed_valves"], time_limit
            )
        ) < stats["pressure"]:
            continue

        # if all are done, stop processing this branch
        if all(location == "DONE" for location in current["locations"]) or current[
            "closed_valves"
        ] == ("DONE",):
            continue
        ...
        if any(
            location in current["closed_valves"] for location in current["locations"]
        ):
            new_data = {
        ...
        next_hops = trim_next_hops(
            (
                working_valves[location]["next_hops"]
        ...
        for next_locations in itertools.product(*next_hops):
            ...
            new_data = {
                "minutes": add_travel_minutes(
                    current["minutes"], current["locations"], next_locations
                )
            }
            heappush(
                heap,
...
def calc_travel_minutes(locations, next_locations):
    ...
    return tuple(
        shortest_path(location, next_location)
...
def get_ordered_next_hops(valve_id):
    ...
    ordered_next_hops = sorted(
        next_hops, key=lambda hop: distances.get(hop, float("inf"))
    )
    ordered_next_hops = [
        hop for hop in ordered_next_hops if valves[hop]["flow_rate"] > 0
    ]
...
def solve(input_value, part):
    ...
    working_valves["DONE"] = {"next_hops": ["DONE"]}
    # part 2
    # 2084 too low
    # 2122 too low
    # 2123 too low
    ...
    pressure, _ = max_pressure_relief(
        working_valves, time_limit=time_limit, workers=workers
    )
    return pressure
